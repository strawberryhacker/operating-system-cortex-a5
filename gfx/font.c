...
struct font_glyph mono_12pt_glyphs[] = {
    {0, 0, 0, 14, 0, 1},        // 0x20 ' '
    {0, 3, 15, 14, 6, -14},     // 0x21 '!'
    {6, 8, 7, 14, 3, -14},      // 0x22 '"'
...
struct font mono_12pt = {mono_12pt_data, mono_12pt_glyphs,
    0x20, 0x7E, 24};

void draw_char(struct rgb* fb, u16 width, u16 x, u16 y, char c)
{
    struct rgb (*buffer)[width] = (struct rgb (*)[width])fb;
    const struct font* font = &mono_12pt;

    u8* bitmap = font->bitmap;

    struct font_glyph* tmp = font->glyph + c - font->first; 
    u8 w = tmp->width;
    u8 h = tmp->height;
    ...
    for (u8 i = 0; i < h; i++) {
        ...
        for (u8 j = 0; j < w; j++) {
            ...
            if (bits & 0x80) {
                print("%d %d %d %d\n", x, y, xo, yo);
                buffer[y + yo + 1][x + xo] = (struct rgb){0, 0b11110000, 0};
                buffer[y + yo + 1][x + xo + 1] = (struct rgb){0, 0b11110000, 0};
                buffer[y + yo][x + xo + 1] = (struct rgb){0, 0b11110000, 0};
                buffer[y + yo][x + xo] = (struct rgb){0, 0b11110000, 0};
            }
            bits <<= 1;
        }
    }
